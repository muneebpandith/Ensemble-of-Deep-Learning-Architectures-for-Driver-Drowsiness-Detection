...
def saveifcanbesaved(savingas,img,typeofimg,boundingbox):
        BOUNDINGBOXERROR=0
        SAVESUCCESS=0
        if bounding_box[0] < 0:
            bounding_box[0]=0
        ...
        height,width,channels= imgCroppedFace.shape
	...
	if faces != []:
		for person in faces:
			...
			if (os.path.exists(savingas) and os.stat(savingas).st_size> 0):
				...
				print(savingas)
				if saveifcansave(savingas,img,"FACE",bounding_box)==0:
					ERRORFLAG="FACEERROR"
			...
			if (os.path.exists(savingas) and os.stat(savingas).st_size> 0):
				...
				print(savingas)
				if saveifcansave(savingas,img,"EYES",bounding_box_Eyes)==0:
					ERRORFLAG="EYESERROR"
			...
			if (os.path.exists(savingas) and os.stat(savingas).st_size> 0):
				...
				print(savingas)
				if saveifcansave(savingas,imgCroppedEyes,"LEFTEYE",bounding_box_LEye)==0:
			...
			if (os.path.exists(savingas) and os.stat(savingas).st_size> 0):
				...
				print(savingas)
				if saveifcansave(savingas,img,"MOUTH",bounding_box_Mouth)==0):
